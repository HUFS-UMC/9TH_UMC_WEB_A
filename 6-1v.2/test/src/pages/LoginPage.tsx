...
const LoginPage = () => {
  const { login, accessToken} = useAuth(); // auth 안에는 { accessToken, login, logout }이 들어 있는데 그 중 login가져오기
  const navigate = useNavigate(); //React Router의 페이지 이동 함수 및에 navigate (my) 로사용


//이 코드의 의도:이 코드는 "로그인 페이지에 들어온 사용자가 이미 로그인돼 있다면, 로그인 페이지를 못 보게 막자"
// 즉, ‘로그인된 사용자가 /login 페이지 접근 금지’
  useEffect(() => {
    if (accessToken) {
      navigate("/"); 
    }
  ...
  const { values, errors, touched, getInputProps } =
    useForm<UserSigninInformation>({
      // 제네릭 타입으로 이 폼이 다룰 데이터의 형태를 고정함여기서는 { email: string; password: string }
