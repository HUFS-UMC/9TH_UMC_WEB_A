...
interface WelcomeDataType {
  id: number;
  name: string;
  email: string;
}

const WelcomeData = () => {   // ✅ const 그대로 두고
...
export default WelcomeData; // ✅ default export로 변경 (중괄호 없이 불러올 수 있음)

export const UserDataDisplay = ({ userId }: { userId: number }) => {
  const { data, isPending, isError } = useCustomFetch<WelcomeDataType>(
